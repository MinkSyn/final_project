                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        if c == 2:
                            ...
                            if last_layer == 'all_KNN':
                                result, distance = all_KNN(im0[y:y+h,x:x+w], model=resnet, emb_face=emb_face, threshold=0.75)
                                label = result+', '+str(float(distance[0]))[:5]
                            elif last_layer == 'fully':
                                result = classify_FC(im0[y:y+h,x:x+w], resnet=resnet, net=emb_face, class_train=class_train, threshold=15.)
                                label = result
...
if __name__ == "__main__":#/models/parameter/YOLOv5
    run(
        weights=ROOT + '/data/weight_train_v2.pt',  # model path or triton URL
